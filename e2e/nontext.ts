...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * gradient whose worst stop spans its element, which is not true of the
     * `.tour` stage here: its `linear-gradient(160deg, …, var(--surface) 62%)`
     * runs the height of a card several hundred pixels tall, so the eyebrow at
     * the top sits on the accent-tinted end and the stage columns at the
     * bottom sit on flat `--surface`. So each gradient is *sampled* at the
     * text's real location instead: linear by projecting onto the gradient
     * line, radial by distance from the centre over the farthest-corner
     * radius. A non-gradient layer is unmeasurable and paints nothing — which
     * is what the `<select>` chevron `url()` and the dashed legend swatch's
     * `currentColor` band resolve to, correctly, since neither sits under text.
     * The radial branch has nothing to do on this page; it is kept because it
     * is the same three lines either way and a gradient added later would
     * otherwise be silently mismeasured rather than loudly unsupported.
     */
     ...
     *
     * A driven scan here walks eleven exhibits at once — the accumulation
     * chain, every hex block, the set list, both comparison tables, the chart's
     * SVG labels and the prediction scoreboard — and every one of those nodes
     * re-walks the same handful of ancestors up to `<body>`. Without the caches
     * the pass re-reads the same computed styles and rects tens of thousands of
     * times, and the gate scans thirty-odd states per configuration. Nothing
